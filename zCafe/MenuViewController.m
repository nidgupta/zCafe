...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary* oneitem = [self.menuItems objectAtIndex:indexPath.section];
    
    cell.cafeOrderButton = [UIButton js_defaultPlaceOrderButton_iOS6];
    cell.cafeTitleLabel.text = [oneitem objectForKey:@"title"];
    cell.cafeDescriptionLabel.text = [oneitem objectForKey:@"description"];
    ...
    [cell.cafeOrderButton addTarget:self action:@selector(orderButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
    cell.cafeOrderButton.tag=indexPath.section;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"placeOrderFromMenu"]) {
        
        UIButton *senderButton = (UIButton *)sender;
        NSInteger rowNum = senderButton.tag;
//        NSIndexPath *indexPath = [self.tableView indexPathForSelectedRow];
//        NSDictionary* oneitem = [self.menuItems objectAtIndex:indexPath.section];
        NSDictionary* oneitem = [self.menuItems objectAtIndex: rowNum];
